...
const assertAlignment = (qr, centerColumn, centerRow) => {
  for (let deltaRow = -2; deltaRow <= 2; deltaRow += 1) {
    for (let deltaColumn = -2; deltaColumn <= 2; deltaColumn += 1) {
      ...
      assert.equal(
        moduleAt(qr, centerColumn + deltaColumn, centerRow + deltaRow),
        expected,
        `alignment module at ${centerColumn + deltaColumn},${centerRow + deltaRow}`,
      );
    }
  }
};
...
test('alignment patterns are exact and finder-overlapping positions are skipped', () => {
  ...
  assertAlignment(qr, 18, 18);
  assertFinder(qr, 0, 0);
  assertFinder(qr, qr.size - 7, 0);
  assertFinder(qr, 0, qr.size - 7);
});

test('version 7 alignment patterns overwrite intersecting timing modules', () => {
  const qr = buildMatrix(new Uint8Array(totalCodewords(7)), 7, 'M', { mask: 0 });

  assertAlignment(qr, 6, 22);
  assertAlignment(qr, 22, 6);
});
...
test('dataRegionIterator matches the independent remainder-bit table for versions 1 through 40', () => {
  const expectedRemainderBits = [
    0,
    ...new Array(5).fill(7),
    ...new Array(7).fill(0),
    ...new Array(7).fill(3),
  ...
  assert.equal(expectedRemainderBits.length, 40);

  expectedRemainderBits.forEach((expected, index) => {
    const version = index + 1;
    const size = 17 + 4 * version;
    const actual = [...dataRegionIterator(size, version)].length - totalCodewords(version) * 8;
    assert.equal(actual, expected, `version ${version}`);
  });
});
...
// Known matrices generated by Nayuki QR-Code-generator's Python implementation
// at commit 2c9044de6b049ca25cb3cd1649ed7e27aa055138 with byte mode, M, forced
// mask 0, boost disabled, and min/max version pinned to the named version.
for (const vector of [
  {
    version: 7,
    byteCount: 107,
    sha256: '7c4476fe04a3879c36da324b26cd721f6192c12e787f5e50790ad2cc18dd1c4f',
  },
  {
    version: 26,
    byteCount: 998,
    sha256: 'a47a4c9f8c27be2229912d94dfc6d426b93e2ccaf313bc85167b542d1382487c',
  },
]) {
  test(`version ${vector.version} matrix matches the pinned Nayuki mask-0 vector`, () => {
    const bytes = new TextEncoder().encode('A'.repeat(vector.byteCount));
    const codewords = encodeToCodewords(bytes, vector.version, 'M');
    const qr = buildMatrix(codewords, vector.version, 'M', { mask: 0 });
    const digest = createHash('sha256').update(qr.modules).digest('hex');

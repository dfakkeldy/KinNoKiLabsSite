...
- **Web Tools** — `/tools` is the hub for the eight theme-routed utilities at `/tools/qr-code`, `/tools/epub-reader`, `/tools/dilution`, `/tools/contrast`, `/tools/word-count`, `/tools/unit-converter`, `/tools/click-counter`, and `/tools/passphrase`, plus the exact-pinned React build at `/tools/turn-timer/`. Theme-routed pages load `Resources/tools/ui.js`, which dynamically imports only that page's DOM-free engine and `*-ui.js` controller; `core.js` provides shared browser-safe UI, storage, connectivity, and service-worker helpers. A `/tools/`-scoped service worker plus `manifest.webmanifest` precaches the hub, the eight theme-routed utilities, and required shared assets for offline use and installation. Turn Timer carries its own nested manifest and service worker scope; update it only with `make sync-turn-timer-web`, which validates the clean VisualTimer source checkout against `Tools/turn-timer-web-source.json` before atomically replacing `Resources/tools/turn-timer/`. Preferences use the single localStorage key `kinnoki-tools:v1`; the on-device EPUB library uses IndexedDB database `kinnoki-tools-epub`; nothing is uploaded. `wordlist.js` includes the Electronic Frontier Foundation Large Wordlist under CC BY 3.0 with source attribution in the file and passphrase page. Run `make test-tools` after Web Tools changes.
